...
void curl_set_header(struct CurlTransaction *curltran, char* header)
{
  curltran->headers = curl_slist_append(curltran->headers, header);
}

int curl_perform(struct CurlTransaction *curltran)
{
  curl_easy_setopt(curltran->handle, CURLOPT_HTTPHEADER, curltran->headers);
  return curl_easy_perform(curltran->handle);
}

long curl_status(struct CurlTransaction *curltran)
{
    long http_code = 0;
    curl_easy_getinfo(curltran->handle, CURLINFO_RESPONSE_CODE, &http_code);
    return http_code;
}
